...
- **Scrollable list runtime API**: `setItems(newItems, ?selectedIndex)` replaces content at runtime (force-applies selection visual even if index unchanged); `scrollToIndex(idx)` scrolls to make item visible; `scrollToAndSelect(idx)` combines scroll + select; `clickMode` (`SingleClick`/`DoubleClick`) controls action event; `disabled` dims list (alpha 0.5) and shows selected in disabled variant. Events: `UIClickItem` (single-click mode), `UIDoubleClickItem` (double-click mode). Setting: `clickMode => "single"` or `"double"`.
