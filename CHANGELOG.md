...
- **`ScreenShakeHelper`** — lightweight additive screen shake helper for impact feedback. Supports basic shake with linear decay, directional shake (horizontal recoil, vertical landing), and custom decay curves. Multiple concurrent shakes stack naturally. Deterministic jitter via time-seeded hash.
