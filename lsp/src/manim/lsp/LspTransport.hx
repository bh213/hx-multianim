...
class LspTransport {
	...
	// Node.js process reference (lazy init)
	...
	public function start():Void {
		nodeProcess = js.Syntax.code("process");
	...
	public function send(msg:Dynamic):Void {
		// Strip null values from nested objects (vscode-languageclient chokes on e.g. children:null)
		// but keep top-level 'result' key even when null (LSP requires it in responses)
		final json:String = js.Syntax.code(
			"JSON.stringify({0}, function(k, v) { return (v === null && k !== '' && k !== 'result') ? undefined : v; })",
			msg);
