...
class ManimAnalyzer {
	...
	public static function getDiagnostics(text:String, uri:String):Array<LspDiagnostic> {
		...
		} catch (e:MultiAnimParser.InvalidSyntax) {
			diagnostics.push({
		...
		} catch (e:MultiAnimParser.MultiAnimUnexpected<Dynamic>) {
			diagnostics.push({
				range: posToRange(e.pos, text),
				severity: DiagnosticSeverity.Error,
				message: e.message,
	...
	static function getChildSymbols(node:MultiAnimParser.Node, text:String):Null<Array<LspSymbolInformation>> {
		final children:Array<LspSymbolInformation> = [];
		for (child in node.children) {
			switch (child.updatableName) {
				case UNTName(name) | UNTNameIndexed(name, _) | UNTNameIndexed2D(name, _, _):
					final range = findNameInText(text, '#$name');
	...
	static function nodeTypeName(type:MultiAnimParser.NodeType):String {
		return switch (type) {
	...
	static function defTypeName(type:MultiAnimParser.DefinitionType):String {
		return switch (type) {
			case PPTInt: "int";
