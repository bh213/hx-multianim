...
import wt.base.filters.PixelOutline.PixelOutlineFilterMode;
...
import wt.stateanim.AnimParser.AnimationStateSelector;
import wt.base.Hex;
import wt.base.Hex.HexLayout;
...
@:using(wt.multianim.MultiAnimParser)
...
typedef TextDef = {
	...
	var lineBreak:Bool;
	var dropShadowXY:Null<wt.base.FPoint>;
...
class MultiAnimParser extends hxparse.Parser<hxparse.LexerTokenSource<MPToken>, MPToken> implements hxparse.ParserBuilder {
	...
	public static function parseFile(input:byte.ByteData, sourceName:String, resourceLoader:wt.base.ResourceLoader) {
		try {
...
//		 trace('PARSE NODES: ${peek(0)}');
	...
	function parsePoint():wt.base.Point {
		switch stream {
	...
	function parseFPoint():wt.base.FPoint {
		switch stream {
	...
	@:allow(wt.multianim.MultiAnimBuilder)
	...
	function parseNode(updatableNameType:UpdatableNameType, parent:Null<Node>,  inheritedDefinitions:ParametersDefinitions, uniqueId:Int):Node {
		...
		final node:Node = switch stream {
			...
			case [MPIdentifier(_, MPText, ITString), MPOpen, fontname = parseStringOrReference(), MPComma, text = parseStringOrReference(), MPComma, color = parseColorOrReference()]:
				...
				var dropShadowXY:Null<wt.base.Point> = null;
