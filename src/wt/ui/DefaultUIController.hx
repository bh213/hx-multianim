package wt.ui;

import wt.ui.UIElement;
import h2d.col.Point;


@:nullSafety
class DefaultUIController extends wt.ui.controllers.UIControllerBase  {
    
    public function new(integration) {
        super(integration);
    }



	public function getDebugName():String {
		return "default UI controller";
	}

    function getEventElement(pos):Null<UIElement> {
        if (controllable.draggable.target != null) return controllable.draggable.target;
        for (element in integration.getElements(SETReceiveEvents)) {
            if (element.containsPoint(pos)) return element;
        }
        return null;

    }
	
}
	
