...
import wt.ui.screens.UIScreen.LayersEnum;
import wt.ui.screens.UIScreen;
import wt.base.PositionLinkObject;
import wt.multianim.MultiAnimBuilder.CallbackRequest;
import wt.multianim.MultiAnimBuilder.CallbackResult;
import hxd.Rand;
...
class UIStandardMultiAnimDropdown implements UIElement implements UIElementDisablable implements UIElementUpdatable implements StandardUIElementEvents
    ...
    function new(builder:MultiAnimBuilder, name:String, builtPanel,  items, initialIndex = 0) {
        ...
        if (this.mainPartImages == null) throw 'could not build combo #${name}';
    ...
    function set_currentItemIndex(value:Int):Int {
		if (value < 0 || value >= items.length) throw 'currentItemIndex ${value} is out of bounds [0..${items.length}].';
    ...
    static function buildPanel(builder, panelName, panelListItemName, items, initialIndex) {
        return UIMultiAnimScrollableList.create(builder, DefaultUIElementItemBuilder.create(builder, panelListItemName), panelName, 120, 300, items, 0, initialIndex); // TODO: initial index
    }
	...
	public function onEvent(wrapper:UIElementEventWrapper){
        ...
        switch wrapper.event {
            ...
            case OnEnter:
                if (autoOpen && !isOpen()) {
                    startOpen(); 
                    if (closeOnOutsideClick) wrapper.control.outsideClick.trackOutsideClick(true);
                }
    ...
    public function doRedraw() {
        ...
        if (currentMainPart != null) currentMainPart.remove();
        ...
        if (panelObject != null) updatable.setObject(new PositionLinkObject(panelObject));  // TODO: Set modal layer
	...
	public function customAddToLayer(requestedLayer:Null<LayersEnum>, screen:UIScreen, updateMode:Bool) {
        ...
        if (requestedLayer == null) {
            if (updateMode) throw 'customAddToLayer update mode had no layer';
            else return Postponed;
        }
        if (!updateMode) screen.addObjectToLayer(this.root, requestedLayer);
