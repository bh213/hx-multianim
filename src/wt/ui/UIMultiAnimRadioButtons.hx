...
import wt.ui.UIMultiAnimCheckbox;
...
@:nullSafety
class UIMultiAnimRadioButtons implements UIElement implements UIElementDisablable implements StandardUIElementEvents implements UIElementListValue implements UIElementSubElements {
	...
	function builderCallback(request:CallbackRequest):CallbackResult {
		...
		switch request {
			...
			case PlaceholderWithIndex(name, index):
				...
				} else throw 'invalid callback ${request}';

	
			default: throw 'unsuported callback ${request}';
		}
	}
	...
	public dynamic function onSingleToggle(index:Int, checked:Bool, controllable) {
		...
		if (checked) this.setSelectedIndex(index);
    ...
    public static function create(builder:MultiAnimBuilder, radioButtonsBuildName:String, singleRadioButtonBuilderName:String, items:Array<UIElementListItem>, selectedIndex:Int) {
