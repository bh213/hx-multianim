...
import wt.multianim.MultiAnimBuilder.BuilderResult;
import wt.multianim.MultiAnimMultiResult;
import wt.base.MAObject;
import wt.ui.UIElement;
import wt.multianim.MultiAnimBuilder.MultiAnimBuilder;
import h2d.Object;
import h2d.col.Point;
import wt.ui.UIElement.UIElementEvents;


//var allStates = hover, pressed, disabled, normal, selected

class UIMultiAnimScrollableList implements UIElement  implements StandardUIElementEvents implements UIElementSyncRedraw implements UIElementUpdatable implements UIElementListValue {
    ...
    function buildPanel() {
        ...
        var builtPanel = panelBuilder.buildWithParameters(panelName, ["width"=>width, "height"=>height, "topClearance"=>topClearance],{placeholderObjects: ["mask" => PVObject(mask)]});
        root.removeChildren();
    ...
    function updateScrollbar() {
        if (this.scrollbar != null) this.scrollbar.remove();
    ...
    function findInteractiveIndex(point):Null<MAObject> {
        for (object in interactives) {
            if (object.getBounds().contains(point)) return object;
        }
        return null;
    }
    ...
    function parseInteractiveId(obj:MAObject):Int {
        switch obj.multiAnimType {
            case MAInteractive(width, height, identifier):
                ...
                if (ident == null) throw 'could not parse interactive id ${identifier}';
                return ident;
        }
    }
    ...
    public function doRedraw() {
        ...
        if (hoverItem != null) hoverItem.remove();
        if (selectedItem != null) selectedItem.remove();
        if (pressedItem != null) pressedItem.remove();

        var stateName = "normal";
        if (currentHoverIndex == currentItemIndex) stateName = "hover";
        if (currentPressedIndex == currentItemIndex) stateName = "pressed";
    ...
    public function onEvent(wrapper:UIElementEventWrapper){
        ...
        switch wrapper.event {
            ...
            case OnKey(key, release):
                if (key == hxd.Key.UP) keyScrollingUp = !release;
                if (key == hxd.Key.DOWN) keyScrollingDown = !release;
    ...
    function set_currentPressedIndex(value:Int):Int {
		if (value < -1 || value >= items.length) throw 'currentPressedIndex ${value} is out of bounds [-1..${items.length}].';
    ...
    function set_currentHoverIndex(value:Int):Int {
		if (value < -1 || value >= items.length) throw 'currentHoverIndex ${value} is out of bounds [-1..${items.length}].';
    ...
    function set_currentItemIndex(value:Int):Int {
		if (value < -1 || value >= items.length) throw 'currentItemIndex ${value} is out of bounds [0..${items.length}].';
